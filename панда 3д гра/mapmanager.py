...
class Mapmanager1():
    def __init__(self, base):
        self.base = base
        self.model = 'metal_sonic_remake.obj'  # Модель кубика
        self.land = self.base.render.attachNewNode("Land")  # Створення вузла для "землі"

    def addMetal(self, position):
        # Завантаження моделі та текстури
        self.metal = self.base.loader.loadModel(self.model)  # Використовуємо self.base.loader
        self.metal.setPos(position)
        self.metal.reparentTo(self.land)
       # Додаємо блок до "землі"